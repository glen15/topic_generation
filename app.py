...
def generate_streamlit_prd(idea_content):
    ...
    prompt = f"""
다음 해커톤 아이디어를 바탕으로 Streamlit 앱 구현을 위한 간단한 PRD를 Markdown 형식으로 작성해주세요.
...
### 사용할 Streamlit 컴포넌트
- 입력: `st.text_input()`, `st.selectbox()` 등
- 출력: `st.write()`, `st.chart()` 등
- 상호작용: `st.button()`, `st.tabs()` 등
...
**간단하고 실용적으로 작성하되, 실제 구현 가능한 내용으로 해주세요.**
"""
